...
## 工業統計調査 市区町村別 産業中分類別の製造業統計（manufacture スキーマ）

| テーブル | 内容 | 主なカラム |
|---------|------|-----------|
| municipality_industry | 都道府県・市区町村・区別、産業中分類別の事業所数・従業者数・製造品出荷額等 | survey_year / amount_reference_year / area / industry_code / establishments / employees / shipment_value_10k_yen |

工業統計調査 確報の「市区町村別、産業中分類別統計」です。従業者4人以上の事業所が対象で、
2008年から2020年までの10年分を収録しています。経済センサス‐活動調査が5年に1度の全産業の
断面なのに対し、こちらは製造業だけを毎年数えます。市区町村×産業中分類の製造品出荷額等が
毎年そろうのはこの調査だけです。

### 件数と金額で対象期間が違う

2017年調査から調査期日が12月31日から翌年6月1日に変わり、事業所数・従業者数は調査年の
6月1日現在、現金給与総額・製造品出荷額等などの経理事項は前年1〜12月の実績になりました
（2020年確報の製造品出荷額等は2019年実績です）。2014年までは12月31日現在と当年1〜12月の
実績で、調査年と実績年が同じです。

同じ1行の中で件数と金額の対象期間がずれるので、金額を年で並べるときは survey_year ではなく
amount_reference_year を使います。金額の実績年は 2008〜2010・2012〜2014・2016〜2019 で、
2015年の実績はどの年にも入りません。

### 年の欠けと分類の切れ目

2011年・2015年・2016年は経済センサス‐活動調査の実施年で、工業統計調査が行われていません。
2020年確報が最後で、以降この調査は行われていません。

2007年以前は収録していません。日本標準産業分類 第12回改定（2008年適用）で繊維工業と
衣服製造業が統合され、以降の中分類コードが1つずつ繰り上がったためです。同じコードが
2007年と2008年で別の産業を指し（26 は2007年が一般機械器具製造業、2008年以降が
生産用機械器具製造業）、コードで時系列を引くと黙って別の産業が混ざります。

### 産業中分類の内訳を持つ地域

内訳が入るのは市区町村と区の行だけです。都道府県・政令指定都市・東京特別区の行は
製造業計（industry_code = '00'）しか持ちません。政令指定都市の内訳はその区の行にあります。
事業所の少ない市町村も内訳が秘匿されていて入りません。

2020年で内訳を持つのは市区町村772・区198の計970地域で、その事業所数の合計は159,720。
全国181,876事業所の88%にあたります。内訳の無い市町村は929で、事業所数の中央値は16です。
都道府県の産業別内訳は同じ調査の別の統計表（都道府県別、東京特別区・政令指定都市別の
産業中分類別統計）にあり、この表には入っていません。

### 地域の粒度

全国の行はありません。47都道府県の和が全国計に一致します（2008年の事業所数 263,061 で、
同じ調査の産業編「従業者4人以上の事業所に関する統計」の全国計と同値）。

| area_level | 内容 | 地域数（2020年） |
|-----------|------|---------------:|
| prefecture | 都道府県 | 47 |
| municipality | 市・町村・東京特別区（13100） | 1,701 |
| ward | 政令指定都市の行政区・特別区 | 198 |

日本全域をちょうど1回覆うのは prefecture だけ、または municipality だけです。ward は
municipality の内訳で、足すと二重に数えます。23区の合計にあたる東京特別区（13100）は
municipality に置いています。e-Stat のメタ情報では2013年以降これが区と同じ階層に入って
いるため、メタ情報の階層をそのまま使うと東京が丸ごと欠けます（2020年の全国計が
181,876 ではなく 174,426 になります）。

地域の名称は年で書き方が変わります。2012年までは政令指定都市の区が市名を含まず（中央区）、
2013年以降は含みます（札幌市中央区）。年をまたぐ集計は名称ではなく area で束ねます。

### 足せる項目と足せない項目

事業所数と従業者数は欠測がなく、産業中分類24区分の和が製造業計に一致します（内訳を持つ
9,646の地域×年すべてで不一致0）。市区町村の和も都道府県に一致します（470の都道府県×年の
うち469。長野県の2020年だけ都道府県が4,767事業所・202,222人、市区町村の和が4,766事業所・
202,218人で、1事業所ぶんずれます）。政令指定都市の区の和はその市に全204組で一致します。

金額の項目（現金給与総額・原材料使用額等・製造品出荷額等・粗付加価値額）は事業所が少ない
区分を秘匿するため NULL が入り、値が入るのは全体の70%です。製造品出荷額等の中分類の和は
製造業計の中央値で94%にとどまり、9,625の地域×年のうち一致するのは109だけです。総数を
出したいときは内訳を足さず、製造業計の行・都道府県の行をそのまま読みます。

```sql
-- 製造品出荷額等が多い市区町村（2019年実績 = 2020年調査。内訳ではなく製造業計を読む）
...
```sql
-- 豊田市の事業所数の推移（産業中分類別、2008年調査と2020年調査）
-- 政令指定都市を同じように引くと空になる。内訳はその区の行にある
SELECT industry_name,
       MAX(establishments) FILTER (WHERE survey_year = 2008) AS y2008,
       MAX(establishments) FILTER (WHERE survey_year = 2020) AS y2020
FROM e_stat.manufacture.municipality_industry
WHERE area = '23211' AND industry_level = 2
GROUP BY industry_name
ORDER BY y2020 DESC NULLS LAST
LIMIT 10;
```

その他収入額（other_income_10k_yen）と有形固定資産年末現在高
（tangible_fixed_assets_10k_yen）は2014年までの統計表にしかなく、2017年以降は NULL です。
粗付加価値額は負の値をとる行があります。

原典が「-」（該当数字なし）・「X」（秘匿）の区分は NULL になり、2つの意味は区別できません。

経済センサス‐活動調査（economic_census.establishment_industry）と単純につなげられません。
あちらは全産業を産業大分類で見た2021年の断面で、事業所の規模を絞りません。こちらは製造業だけを
産業中分類で見た毎年の系列で、従業者4人以上に限ります。同じ「製造業の事業所数」でも母数が
違います。

出典: 経済産業省 工業統計調査 確報 市区町村編／地域別 市区町村別、産業中分類別統計表。
https://www.e-stat.go.jp/
...
市区町村ごとに、何人が入ってきて何人が出ていったかを毎年数えた表です。住民票の異動届に
もとづく実績なので、将来推計人口と違って推計値ではありません。2010年から2025年までの
210,927行です。

| テーブル | 内容 | 主なカラム |
|---------|------|-----------|
| municipality_migration | 全国・都道府県・市区町村・区別の転入者数・転出者数・転入超過数（年次） | area / area_level / year / sex_code / nationality_code / inflow / outflow / net_inflow |

市区町村をまたぐ移動だけを数えるので、同一市区町村内の引っ越しと国外との出入りは入りません。
`net_inflow` は `inflow - outflow` に等しく、負の値は転出超過です。全国の転入超過数は、どの
移動も転入と転出の両方に数えられるため定義上 0 になります。

### 地域の粒度

`area_level` は national（1）・prefecture（47）・municipality（市・町村・政令指定都市・
東京都特別区部、通期で1,736）・ward（政令指定都市の区と特別区、201）・county（郡・北海道の
振興局・東京の支庁、332）・urban_rural_part（市部・郡部、96）です。日本全域をちょうど1回
覆うのは municipality だけで、ward はその内訳、county は町村の合計、urban_rural_part は
市と町村を別の切り方でまとめた合計です。絞らずに合計すると何重にも数えます。

地域数は年によって変わります。2025年は county と urban_rural_part の行がありません。

```sql
-- 転入超過の多い市区町村（2025年）
SELECT area_name, inflow, outflow, net_inflow
FROM municipality_migration
...
`nationality_code` は日本人移動者（61000）が2010年から、外国人を含む移動者（60000）が
2018年から、外国人移動者（62000）が2020年からです。2017年以前まで遡る時系列は 61000 で
引きます。60000 で引くと2018年から始まる系列になり、2017年と2018年の間に外国人の分だけ
段差ができます。

### 年齢階級の内訳は持っていない

原典には年齢別の表もありますが、年齢区分が年代で違います（2019年以前は3区分、2020年以降は
5歳階級）。同じ列に積むと区分の意味が年で変わるため、年齢は総数だけを収録しています。

### resident_registry.population の転入・転出との違い

同じ住民票の異動から作られる数が `resident_registry.population` にもあります
（`moved_in_domestic` / `moved_out_domestic`）。対象期間は2014年以降どちらも暦年で同じですが
（住基のほうは調査期日1月1日の直前1年間なので、`year = 2026` の行が2025年にあたります）、
別々に集計された統計なので値は一致しません。2025年の全国・日本人の国内転入は、この表が
4,528,254人、住基の動態が4,523,542人です。全国で転入と転出が一致するのはこの表だけで、
住基の動態は同じ2025年の全国・日本人で国内転入4,523,542人・国内転出4,544,769人と差が残ります。

### 欠測値

原典が欠測値としている市区町村と年があります。転出者数が入らないのは矢祭町（07482）の
2010年〜2014年と国立市（13215）の2010年〜2011年、転入超過数が入らないのはその翌年まで
（矢祭町は2015年、国立市は2012年）です。転入者数はどの年もあります。都道府県と全国の値も
その分を欠いたまま積まれているので、市区町村の和は都道府県の値と一致します。

政令指定都市に移行した年だけは、市の値が通年で区の値が4月以降になるため、区の和が市に
届きません（相模原市の2010年・熊本市の2012年）。
...
## 小売物価統計調査 調査品目の年平均価格（retail_price スキーマ）

同じ品物が市によっていくらで売られているかを、指数ではなく円のまま収録した表です。
2000年から2025年までの1,207,312行です。

| テーブル | 内容 | 主なカラム |
|---------|------|-----------|
| item_price | 都道府県庁所在市及び人口15万以上の市の品目別年平均価格 | cat02 / item_name / item_note / area / area_name / year / unit / value |

地域は延べ95コードで、1年に載るのは71〜81市です。47都道府県すべてに1市以上ありますが、
全国計の行はありません。品目は延べ872銘柄で、1年に載るのは534〜693です。市と品目の
組合せの22%は `value` が NULL で、その年その市で調査していないことを表します。
...
2024年の食パンは那覇市の718円が最も高く、函館市の304円が最も安い値です。

### 品目をまたいで価格を比べない

価格は品目ごとに決められた数量（1kg当たり・100g当たりなど）に対する額ですが、その数量は
この表に入っていません。`value` を品目間で並べても意味を持ちません。比べられるのは同じ
品目の市どうしか、同じ品目・同じ市の年どうしです。数量の規格は
[統計局の調査品目及び基本銘柄](https://www.stat.go.jp/data/kouri/doukou/3.html#meigara)にあります。

### 単位が円でない品目がある

`unit` は全行「円」ですが、火災・地震保険料の5品目（3182〜3184, 3186, 3187）は保険料率で、
実際の単位は割合です。見分けられるのは `item_note` の【表章単位：割合】だけです。3186・3187
は2024年までが円、2025年から割合なので、同じ品目コードで単位が切り替わります。

`value` が 0 の行（公立高校授業料や保育料の無償化）と、負の行（電気代の燃料費調整単価）も
あります。

### 調査を終えた品目と市が同じ列に残る

品目名と市名の末尾には【2021年12月調査終了】【2015年01月から3511 電気代】のような注記が
付きます。`item_note` / `area_note` がその注記で、注記の無い行は NULL です。現役の品目だけを
見るときは調査終了の注記を除きます。

### 市のコードは政令指定都市への移行で変わる

`area` は5桁の標準地域コードそのものなので `code.municipality` と直接つながります（95コード中
88件）。つながらない7件は政令指定都市になる前のコードで、`area_note` に有効期間が入ります。
...
市の時系列を通して見るときは、コードではなく市の名称で寄せるか `code.municipality_change`
で対応させます。

### cpi.price_index との違い

`cpi.price_index` は地域ごとに2020年=100 に揃えた指数なので、市どうしの水準は比べられません。
比べられるのは同じ地域の時間変化です。円の水準で市を並べられるのはこの表です。地域コードの
体系も違い、`cpi.price_index` は 01A01 形式、`household.expenditure` は 01003 形式で、どちらも
`area_name` の先頭5桁がこの表の `area` にあたります。
...
## 社会福祉施設等調査 施設数・定員・在所者数・従事者数（welfare_facility スキーマ）

保育所・障害者支援施設・児童養護施設・養護老人ホームなどの施設数・定員・在所者数・
常勤換算従事者数を、都道府県別・施設の種類別・経営主体別に収録した表です。
2011年から2024年までの864,063行で、調査日は毎年10月1日です。

| テーブル | 内容 | 主なカラム |
|---------|------|-----------|
| facility_statistics | 都道府県別・施設の種類別の施設数・定員・在所者数・従事者数 | survey_year / area_kind / area_code / area_name / prefecture_code / facility_code / facility_type / facility_level / operator / survey_form / measure / value |

1セル1行の縦持ちで、指標は `measure` で選びます（`facility_count` 施設数、`capacity` 定員、
`occupants` 在所者数、`fte_workers` 常勤換算従事者数）。値が入るのは年・指標ごとに
34.4%〜63.3%の行で、残りはその年その地域にその種類の施設が無いか、原典が値を伏せた行です。
...
2024年の保育所等の定員は東京都が321,215人で最も多く、愛知県162,053人、
神奈川県156,176人と続きます。

### 都道府県の時系列は市の行を足してから作る

2017年調査までは指定都市と中核市が都道府県とは別の行に並び、都道府県の行はその市の分を
含みません。2018年調査からは市の行が無くなり、都道府県の行が県全体になります。
`area_kind = 'prefecture'` だけで年をまたぐと、2018年に実体のない跳ねが出ます
（有料老人ホームの施設数は 2017年 7,821 → 2018年 14,454）。市の行にはその市が属する
都道府県が `prefecture_code` に入るので、そこで束ねると通しで見られます
（同じ系列が 2017年 13,525 → 2018年 14,454）。

国が設置する施設は `area_kind = 'state'` の行にあり、どの都道府県にも属しません。

### 足し合わせる段は facility_level = 'type'

施設の種類の見出しは段になっていて、そのまま足すと二重に数えます。足し合わせて全体に
なるのは `facility_level = 'type'` の段だけです。`group` は大分類の小計、`detail` は
保育所等・地域型保育事業所を割った内訳、`reprint` は再掲、`total` は全体の総数です。

`facility_level = 'type'` の行で `facility_group` が NULL になるのは、大分類に属さない
婦人保護施設（2024年調査からは女性自立支援施設）だけです。2014年調査までの定員・在所者数
の表は原典が大分類を印字しませんが、施設の種類の符号から同じ年の別の表の大分類を当てて
補ってあります。大分類そのものの行（`group`）・総数の行（`total`）・再掲の一部は
`facility_group` を持ちません。

### 基本票と詳細票で母集団が違う

2012年調査から、全施設に配る基本票と一部が標本の詳細票の2本立てになりました。施設数は
基本票、在所者数と常勤換算従事者数は詳細票、定員は両方にあります。定員は `survey_form` を
指定しないと1つの地域・施設に2行返ります。2011年調査は票が1種類しか無いので NULL です。

### 在所者数が定員を超える施設の種類がある

通所で利用する人を含む施設の種類では、在所者数が定員を上回ります（2024年の全国で、
児童発達支援センターは定員25,043人に対し在所者数47,619人）。母子生活支援施設は定員が
世帯数、在所者数が世帯人員数で、そもそも単位が違います。都道府県 × 施設の種類で見ると、
在所者数が定員を超える組合せは19.4%あります。

### 2023年に幼保連携型認定こども園が対象から外れた

2023年調査から幼保連携型認定こども園が調査の対象外になりました。保育所等の定員は2022年の
2,939,776人から2023年は2,259,096人に、施設数の総数は83,821から77,803に下がります。
減ったのではなく数える範囲が変わったので、2022年をまたぐ比較はこの施設を外してそろえます。

### 総数に入っていない施設がある

定員と在所者数の総数には母子生活支援施設が入りません。2011年調査の定員はさらに助産施設も
外れます。単位が人でない施設もあり、母子生活支援施設の定員は世帯数、有料老人ホーム
（サービス付き高齢者向け住宅であるもの）の定員は戸数、助産施設の定員は認可病床数です。
...
年・指標ごとに51.7%〜72.3%の行で、残りはその年その市区町村にその種類の施設が無い行です。

```sql
-- 介護老人福祉施設の定員が多い市区町村（2024年）
...
2024年の介護老人福祉施設の定員は横浜市が18,037人で最も多く、大阪市13,715人、
名古屋市8,439人と続きます。全国1,741市区町村のうち、介護老人福祉施設があるのは1,630です。

### 行政区の行を足すと二重に数える

政令指定都市は市の合計行とその行政区の行が両方並びます。足し合わせて全国になるのは
`area_kind` が `municipality` と `designated_city` の行だけです。行政区（`ward`）を混ぜると
2024年の介護老人福祉施設の定員が全国604,469人に対して724,785人になります。

東京23区は基礎自治体なので `municipality` です。その集計行「特別区」（13100）は
2004年〜2010年調査にだけ並び、`district` になります。

### 地域コードは調査時点のもの

`area_code` は調査した年の標準地域コードです。平成の大合併で消えたコードも入っているので、
2001年〜2024年調査に出る3,785コードのうち `code.municipality` に残るのは1,960です。
年をまたいで市区町村を追うときは `code.municipality_change` で新旧のコードをつなぎます
（収録は2007年4月2日以降の変更のみ）。
...
2012年調査から、全施設を数える基本票と、回収率の変動を受ける詳細票の2本立てになりました。
2017年調査までは両方があり、2018年調査からは基本票だけです。`survey_form` をそろえないと
実体のない段差が出ます（介護老人福祉施設の施設数は2017年調査が詳細票7,299・基本票7,891で、
詳細票から2018年調査の基本票8,097につなぐと1割の跳ねになります）。2011年調査までは票が
1種類しか無いので NULL です。

### 施設の種類は年で入れ替わる

介護医療院は2018年調査から表に出ます（62施設 → 2024年調査で917施設）。入れ替わりに
